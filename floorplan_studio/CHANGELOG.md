...
- The dashboard plan no longer flickers between dawn and dusk. Each time its
  night overlay was redrawn it recalculated the sun to the millisecond, which
  changed the overlay again, so it never settled; it went still only once the
  sun was well down. The plan now draws the sun to the minute, in the editor as
  well as on the dashboard.
- Switching a light no longer makes the plan flicker — after dark, or by day,
  when a faint night still shows. The new night overlay is fully prepared while
  the old one stays on screen, then the two are exchanged in one step, so the
  floor is never shown with no night or with two.

### Pictures in the help, and a dashboard you can press before installing

- The help pages show what they describe: daylight at four hours, lamps at
  three wattages, six floor finishes, wall treatments and wall tops, door
  swings and door sensors, blinds, a sensor's wedge, colour schemes, and stair
  and planting looks. Each is drawn by the app's own renderer from an invented
  demo house, and redrawn whenever the help is regenerated.
- A live demo of the generated dashboard for that house runs in the browser
  against a stand-in Home Assistant: drag the time of day, tap lamps, open a
  room's popup, run a scene. It sits on the help site's dashboard pages and on
  a page of its own, and it is built from the same card bundle the app installs.

### Room actions together; strip controls on the run

- Each room's **Scenes** row now discovers room-matched scripts as well as
  scenes, so both appear together in the popup. The rule remains data in
  `controls.json`; existing stock registries are upgraded narrowly, while a
  deliberately customised scene-only source is preserved.
- Strip and cove lights replace the detached circle marker with a small thick
  capsule on the light run. Its shape and size come from `library.json`, and
  strips now share line-shaped tap geometry between the editor and dashboard.

### The dashboard on a phone, as tidy as the old card

- Tapping a room with a cove or strip light in it opens the room again. The
  light's tap area had been the whole room inside it, so the room itself could
  not be reached.
- A room's controls rise from the bottom of the screen as a sheet over the
  page, instead of filling the plan card. On a phone the sheet no longer sits
  half off the screen or hides its own header, its tiles fit three to a row,
  and it no longer jumps each time something in it is switched.
- The whole floor plan fits on screen without scrolling, on a phone as on a
  wide screen; the floor snapshot sits under it and scrolls. `sticky: true` on
  the floor card docks it instead.
- Tighter type and spacing on a phone.

### An open editor no longer undoes changes made elsewhere

- An editor that had missed a change made somewhere else — by an assistant
  over MCP, or in another tab — saved its older copy over it the next time
  anything changed, silently undoing it. A save now says which version of the
  plan it started from. If the plan has moved on, the editor merges your edits
  with the newer plan, object by object, and saves that; where you both changed
  the same thing your version is kept and a notice says so.
- An editor also catches up after its live connection drops, as it does while
  the add-on restarts, instead of missing whatever changed meanwhile.
- The rotate handle stays on the drawing. For something against the edge of
  the floor, like a solar array along the top of a roof, it was drawn outside
  the canvas where it could not be grabbed.

### The editor holds still too, and a solar array can be turned

- The editor draws its plan the way the dashboard now does — no blend modes,
  no masks, the night's dark drawn once into an image — so it no longer tears
  or blanks while you scroll and zoom on a desktop graphics card.
- Clicking a solar array selects the array, so its rotate handle turns the
  panels. A solar marker's dashboard tap area covers its whole array, and in the
  editor that stood in front of the array: every click picked the small marker
  and only its icon turned. The marker is now picked on its own icon in the
  editor; on the dashboard, tapping anywhere on the array still opens it.

### A dashboard plan that holds still on a desktop GPU

- The dashboard plan no longer tears, blanks or half vanishes while it
  scrolls and zooms in a desktop browser, which got worse with every lamp lit.
  It now draws its light without CSS blend modes, and holds light to its rooms
  with clip outlines instead of masks. The night's dark, cut by every lit
  lamp, is drawn once into an image whenever the lights change. Glows, lamp
  tints and daylight paint as plain washes, a little softer than in the editor,
  and a lamp's glow no longer spills through a window into the room beyond.
  `compositing: blend` on the plan card or the dashboard puts full
  compositing back.
- A reading that changes every few seconds — a plug's watts, a sensor —
  repaints only its own marker or label instead of the whole plan, and a
  spinning fan keeps spinning when something else on the floor updates.

### Every device turns

- Setting a device's facing now turns its drawing whatever look it has. Solar
  arrays, curtains and blinds, inverters and batteries, routers, washers, TV
  boxes, locks, tanks, pumps and the ceiling-cassette AC all drew the same at
  every angle before — 67 looks in all.
- While you zoom or pan the plan, a change from Home Assistant waits until the
  view settles instead of redrawing the whole plan under your fingers.

### The dashboard on a phone, a tablet and a wide screen

- On a screen at least 900px wide the plan card narrows until the whole floor
  fits the window's height, as the old dashboard's did, instead of running
  taller than the screen.
- The plan's header has the old dashboard's zoom control: **−**, the zoom
  level, **+**, and **⤾** to put a zoomed plan back.
- Zooming no longer tears the plan or leaves holes in it mid-gesture. It moves
  the plan as already painted and repaints it sharp once the gesture stops, so
  walls and labels now grow with the zoom. A pinch is no longer cut short when
  a light elsewhere on the floor changes state.
- A plain scroll over a fitted plan scrolls the page again; `Ctrl`/`Cmd`+scroll
  or a pinch zooms, and a plain scroll pans a zoomed plan.
- The floor snapshot stays docked to the bottom of the screen while the page
  scrolls, beneath an open room sheet rather than over it. `sticky: false` on
  the floor card turns that off.
- The frosted-glass style no longer blurs what is behind the whole plan card,
  which made scrolling flicker; the house and floor cards keep their glass.
- A marker no longer keeps blinking after its command has landed. When Home
  Assistant confirmed exactly the state the tap had already drawn, the "in
  flight" pulse could stay on until something else on the floor changed — most
  noticeably on a fan. A fan's marker also pulses while a command to its
  built-in light is in flight.

### Light that carries as far as the lamp throws it

- A lamp's light is worked out from its **wattage**: brightest under the
  fitting, fading with distance, and in a large dark space still faintly
  visible well away from it. A 5 W downlight and a 30 W flood no longer draw
  the same pool. A fitting with no wattage uses its type's.
- At night light **cuts the dark** rather than being painted over it, so a lit
  floor looks like the floor — marble stays marble — with the lamp's colour on
  it and a soft glow at the fitting. Two lamps are brighter than one, but not
  twice as bright.
- **No flat lit rectangles anywhere.** A walled room still fills evenly with
  light off its walls and ceiling, stopped by the walls; in an open space the
  light fades out instead of ending in a line. Light leaving through a window
  lands on the path outside and stops at the compound wall.
- With **Model artificial light** off, a lit fitting simply glows.
- Every setting in **◐ Light** now does something, and the list the dialog is
  built from is the same list the help's settings table is generated from. The
  "When a room has both sun and lamps" choice, which did nothing, is gone.
- Blend modes were written in a form browsers ignore, so every "brighten" wash
  in the plan had been painting as a flat translucent veil. They now blend.

### Daylight from the side, and covered car ports

- A window or an open side lets daylight in **brightest at the edge and fading
  with depth**, and it carries on across open edges into the rest of the space.
  What stands outside counts: an open side facing a compound wall sees less sky,
  from the wall's own height.
- An outdoor room can be **roofed**: set **Open to the sky above** to 0 in its
  panel (Advanced) and a car port under the floor above is lit through its open
  sides instead of like an open yard. **How deep daylight reaches** is in the
  Sun dialog's Advanced settings.
- Rooms drawn overlapping are no longer lit twice where they overlap.
- An overcast noon stays daylight: the night only gathers through dusk.

### House card dialog shows your symbols

- Counts and stats led by an emoji or any other character (`glyph`) showed
  "dot" in the house card dialog, and changing the icon there did nothing on the
  card. Each row now has a **symbol** box beside the icon list: type an emoji to
  lead with it, clear it to use a drawn icon. The icon list is greyed out while a
  symbol is set.

### Walls that stop at the building, and gates that run behind a pier

- A partition that runs out to the building's edge stops at the outside face
  instead of poking a stub past it.
- A sliding door or gate never parks outside the floor: with less wall behind it
  than its own width, the leaf stops at the edge, the way stacking panels do.
- A sliding gate can run on a rail **set back** from its opening (**Rail set back
  into this room** in the opening's panel), so a compound-wall gate slides along
  the house side of the wall and parks behind the pier.

### Fan light kits, stone floors and inlays

- A ceiling fan's built-in light is bound **on the fan** (**Light kit**), not as
  a separate marker: its light comes from the fan, its diffuser lights up in the
  lamp's colour, it counts as one of the room's lights everywhere lights are
  counted or listed, and a long press on the fan opens it.
- Sun patches are only cast through openings that see the sky — no more strip
  of "sunlight" through a doorway between two indoor rooms.
- A room's open side is checked along its length, so daylight from a courtyard
  no longer spills in under the roofed half of the same edge.

### Crema Breccia, and the theme on the room sheet

- New finish **Crema Breccia** (Marble & travertine): cloudy cream stone with
  soft patches, a network of fine rust cracks and pale spots. Any marble finish
  can now set **cloudy patches**, **cracks** and **spots** in the finish editor.
- New **Stone-look tile 4×2, cloudy grey**, and a new **Inlay** generator — a
  floral border round a grid of lattice panels — for a decorative panel set into
  a floor. Draw the inlay as its own part of the room it sits in.
- The room sheet, its buttons and tiles, and the house and floor cards take
  their colours from the dashboard's theme — the same one the plan is drawn in —
  instead of Home Assistant's greys and a fixed amber.
- **Preview the dashboard** shows a whole tab — the house card, the live plan
  and the floor card — full width, with wide, tablet and phone widths.
