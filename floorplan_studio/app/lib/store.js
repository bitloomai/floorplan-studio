...
function upgradeDoc(key, doc) {
  ...
  if (key === 'controls') {
    ...
    /* Version 1 deliberately matched only scenes by room name. Version 2 puts
     * scripts beside them. Migrate only the unmistakable shipped source: a
     * curated section (or even a copied stock section whose explanatory note
     * was changed) remains the user's data. */
    const scenes = doc.default && Array.isArray(doc.default.sections)
      ? doc.default.sections.find((s) => s && s.id === 'scenes') : null;
    const stockCatalogue = scenes && Array.isArray(scenes.sources)
      ? scenes.sources.find((s) => s && s.source === 'catalogue'
        && Array.isArray(s.filter && s.filter.domains)
        && s.filter.domains.length === 1 && s.filter.domains[0] === 'scene'
        && s.filter.match === '@room' && s.filter.matchMode === 'word') : null;
    if (stockCatalogue && typeof scenes._extending === 'string'
        && scenes._extending.startsWith('Scripts are deliberately NOT keyword-matched here')) {
      const freshScenes = (fresh.default.sections || []).find((s) => s.id === 'scenes');
      stockCatalogue.filter.domains = ['scene', 'script'];
      if (freshScenes) {
        scenes._note = freshScenes._note;
        scenes._extending = freshScenes._extending;
      }
      added.push('section:scenes catalogue scripts');
    }
    if (Number.isFinite(fresh.schemaVersion) && (!Number.isFinite(doc.schemaVersion) || doc.schemaVersion < fresh.schemaVersion)) {
      doc.schemaVersion = fresh.schemaVersion;
      added.push('schemaVersion');
    }
  }
...
module.exports = {
  ...
  async writeProject(project, opts) {
    ...
    /* `opts.base`: the `savedAt` the caller's copy was built on. Checked INSIDE
     * the slot, so nothing can land between the check and the rename; a
     * mismatch writes nothing and throws `STALE_PROJECT`. See the PUT handler
     * in server.js for why. A caller with no base replaces as before. */
    const base = opts && opts.base;
    const saved = await enqueue(file, async () => {
      if (base) {
        const current = await readProjectInSlot(file);
        if (current.savedAt && current.savedAt !== base) {
          const err = new Error('The plan changed elsewhere after this copy of it was loaded.');
          err.code = 'STALE_PROJECT';
          throw err;
        }
      }
      return commitProject(file, project);
    });
