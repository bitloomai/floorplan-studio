...
function trimThemes(themesDoc) {
  ...
  for (const [id, t] of Object.entries(themesDoc.themes || {})) {
    ...
    /* `ui` ships too, and is small: it is the room sheet's surface, ink, lines
     * and accent, so the controls round the plan wear the same theme as the
     * plan. Dropped, the sheet fell back to Home Assistant's greys. */
    out.themes[id] = { label: t.label || t.name, name: t.name, plan: t.plan, ui: t.ui };
 ...
 * @param opts  {version, resourceKey, generatedAt}
...
function build(docs, opts) {
  opts = opts || {};
  const data = {
    /* Fixed only by a caller that commits the bundle and checks it for drift
     * (the help site's live demo); an installed card is always stamped now. */
