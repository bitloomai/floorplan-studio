...
async function handleApi(req, res, pathname, query) {
  ...
  if (pathname === '/api/project') {
    ...
    if (method === 'PUT') {
      ...
      /* The revision this save was built on — the `savedAt` the editor last
       * had. If the project was written since (an MCP tool call, another tab)
       * the save is refused with the current project, and the editor merges
       * instead of overwriting. Without this an editor that had missed a change
       * wrote its stale copy over it: the viewing deck's solar array was turned
       * back three times on 2026-09-13. A save naming no base is accepted as
       * before. */
      const base = String(req.headers['x-fps-base'] || '').slice(0, 64) || null;
      try {
        const saved = await store.writeProject(body, { origin, base });
        ...
        if (e.code !== 'STALE_PROJECT') throw e;
        return sendJson(res, 409, { error: e.message, stale: true, project: await store.readProject() });
      }
    }
  }
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    const SHARED = ['annotations.js', 'project-merge.js', 'room-identity.js', 'ui-navigation.js', 'input-actions.js', 'plan-scene.js', 'flooring.js', 'shapes.js', 'sun.js', 'controls.js', 'entity-bindings.js', 'lighting.js'];
