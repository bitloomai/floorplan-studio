...
window.PanelsDashboard = (function () {
  ...
  async function dashboardDialog() {
    ...
    const body = h('div', {},
      ...
      h('div', { style: 'display:flex;gap:8px;align-items:center;flex-wrap:wrap' },
        installBtn,
        h('a', { class: 'btn', href: 'preview.html', target: '_blank', rel: 'noopener' }, 'Preview the dashboard ↗'),
  ...
  /* A chip leads with a drawn icon OR a symbol of the user's own — `glyph`,
   * any character, emoji included — and the glyph wins on the card. The
   * dialog used to show only the icon list, so a count led by 💡 read "dot",
   * and changing the icon visibly did nothing. Both are shown now: the symbol
   * box, and the icon list greyed out while a symbol is set. */
  function iconPick(o) {
    const names = (window.Shapes && Object.keys(Shapes.ICONS)) || ['dot'];
    const select = h('select', {
      style: 'max-width:96px',
      title: o.glyph ? 'The symbol is shown instead. Clear it to use a drawn icon.' : 'Drawn icon',
      disabled: !!o.glyph,
      onchange: (e) => Store.mutate(() => { o.icon = e.target.value; }, 'icon'),
    }, ...names.map((n) => h('option', { value: n, selected: (o.icon || 'dot') === n }, n)));
    const symbol = h('input', {
      type: 'text', value: o.glyph || '', placeholder: 'emoji', maxlength: 8,
      style: 'width:52px;text-align:center', 'aria-label': 'Symbol',
      title: 'Any character or emoji, shown instead of the drawn icon. Leave empty to use the icon.',
      oninput: (e) => {
        select.disabled = !!e.target.value.trim();
      },
      onchange: (e) => Store.mutate(() => {
        const v = e.target.value.trim();
        if (v) o.glyph = v; else delete o.glyph;
      }, 'symbol'),
    });
    return h('span', { class: 'symbol-pick', style: 'display:inline-flex;gap:4px;align-items:center' }, symbol, select);
  }
...
@media (prefers-reduced-motion: reduce) {
  ...
  /* The light model's settings, built from `Lighting.SETTINGS` — the same list
   * the help's settings table is generated from, so a setting cannot exist
   * without a control here, or be documented without existing. Coverage, door
   * arcs and the room-name chips are drawing settings that live beside it and
   * are written here by hand. */
  function lightingDialog() {
    ...
    /* Writes only the key that changed. The merge supplies every default, and
     * copying a whole group into the project would stop it tracking them. */
    const setPath = (path, value) => Store.mutate(() => {
      const L = S.project.lighting = S.project.lighting || {};
      const [group, key] = path.split('.');
      if (!key) { if (value === null) delete L[group]; else L[group] = value; return; }
      const next = Object.assign({}, L[group] || {});
      if (value === null) delete next[key]; else next[key] = value;
      L[group] = next;
    }, 'lighting');
    const remember = (path, value) => {
      const [group, key] = path.split('.');
      if (!key) cfg[group] = value; else cfg[group] = Object.assign({}, cfg[group], { [key]: value });
    };
    const shown = (s, v) => (s.unit === '%' ? `${Math.round(v * 100)}%`
      : s.unit === 'fc' ? `${v} fc` : s.unit === 'x' ? Number(v).toFixed(2) : String(v));

    const control = (s) => {
      const v = Lighting.getPath(cfg, s.path);
      const hint = s.hint ? h('p', { class: 'hint' }, s.hint) : null;
      if (s.kind === 'toggle') {
        return h('div', { class: 'field', 'data-setting': s.path },
          h('label', { class: 'inline' }, h('input', {
            type: 'checkbox', checked: v !== false,
            onchange: (e) => { remember(s.path, e.target.checked); setPath(s.path, e.target.checked); describe(); },
          }), ' ' + s.label), hint);
      }
      if (s.kind === 'range') {
        const val = h('span', { class: 'mono' }, shown(s, v));
        return h('div', { class: 'field', 'data-setting': s.path },
          h('label', {}, s.label, ' — ', val),
          h('input', {
            ...
            oninput: (e) => {
              ...
              remember(s.path, n); val.textContent = shown(s, n); setPath(s.path, n); describe();
            },
          }), hint);
      }
      return h('div', { 'data-setting': s.path }, field(s.label, h('input', {
        type: 'number', step: s.step, min: s.min, max: s.max, value: v ?? '',
        onchange: (e) => {
          const n = e.target.value === '' ? null : Number(e.target.value);
          remember(s.path, n === null ? Lighting.getPath(Lighting.DEFAULTS, s.path) : n);
          setPath(s.path, n); describe();
        },
      }), s.hint));
    };
    const emit = (list) => {
      ...
      for (const s of list) {
        if (s.group !== group) {
          group = s.group;
          body.append(h('div', { class: 'subhead' },
            s.advanced ? `Advanced — ${group.charAt(0).toLowerCase()}${group.slice(1)}` : group));
        }
        body.append(control(s));
      }
    };

    function describe() {
      const spot = Lighting.lampOutput({ props: { watt: 5, beam: 2.4, direct: 1 } }, null, null, cfg);
      preview.textContent = cfg.enabled === false
        ? 'The light model is off: the plan draws no night, and a lit fitting simply glows.'
        : `A room reads fully lit at ${cfg.targetFc} foot-candles — about `
          ...
          + `At night the plan darkens by ${Math.round(cfg.scrim * 100)}%, and a 5 W downlight’s light `
          + `is still visible about ${Math.round(spot.poolFt)} ft from it.`;
    }

    body.append(h('p', { class: 'hint' },
      'Adjust how the house’s light is drawn. Each fitting’s wattage is set on its marker; one that says nothing counts as its type’s.'));
    emit(Lighting.SETTINGS.filter((s) => !s.advanced && s.group !== 'Motion'));
    body.append(
    ...
    emit(Lighting.SETTINGS.filter((s) => !s.advanced && s.group === 'Motion'));
    body.append(h('div', { class: 'subhead' }, ' '), preview);

    /* ---- advanced ----
     *
     * Real settings a house needs none of to look right: light zones and their
     * spill, floor bounce, how far light carries, what an unspecified fitting
     * counts as, and whether the room name carries a live count. The tick is in
     * this dialog's own header as well as the top bar, because the top bar is
     * COVERED while a dialog is open. */
    const A = Panels.dialogAdvanced();
    body.appendChild(Panels.locationTitle('section:lighting.model'));
    A.adv(body, () => {
      emit(Lighting.SETTINGS.filter((s) => s.advanced));

      body.append(h('div', { class: 'subhead' }, 'Advanced — room name chips'));
      ...
      /* Named rooms, rather than a rule of thumb. `chips.hideRooms` is a list
       * of room ids the count never appears on. Drawn as one tick per room
       * across the whole house, because the alternative is typing room ids. */
    ...
    A.note(body, 'light zones, floor bounce, how far light carries, what an unspecified fitting counts as, and the room name chips');
