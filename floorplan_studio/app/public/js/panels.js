...
window.Panels = (function () {
  ...
  function renderRoomPanel(box, floor, room) {
    ...
    adv(box, () => {
      ...
      if (room.outdoor) {
        /* An outdoor room is not always under open sky. A car port or a
         * verandah under the floor above is outdoor and roofed: its daylight
         * comes in from its open sides and fades towards the house. */
        box.appendChild(field('Open to the sky above (blank = use the house)', h('input', {
          type: 'number', step: 0.05, min: 0, max: 1, value: dl.outdoor ?? '',
          placeholder: String((S.project.sun && S.project.sun.ambient && S.project.sun.ambient.outdoor) ?? 1),
          onchange: (e) => Store.mutate(() => {
            const v = e.target.value === '' ? null : Math.max(0, Math.min(1, Number(e.target.value)));
            if (v === null) { if (room.daylight) delete room.daylight.outdoor; } else {
              room.daylight = Object.assign({}, room.daylight, { outdoor: v });
            }
          }, 'room sky'),
        })));
        box.appendChild(h('p', { class: 'hint' },
          '1 is open sky. 0 is roofed — a car port under the floor above — lit only through its open sides, brightest at the edge and fading inwards.'));
      }
    });
