...
window.Canvas = (function () {
  ...
  const el = (tag, attrs, text) => {
    ...
    for (const [k, v] of Object.entries(attrs || {})) {
      ...
      /* A blend mode only works as CSS; as an attribute it is ignored. */
  ...
  /* The night scrim as a bitmap, shared with the dashboard card
   * (PlanScene.scrimBitmaps): one per canvas, so a redraw of the same light
   * reuses the last one. Made on first paint, not at load: this file is also
   * evaluated where PlanScene is not there yet. */
  let scrims = null;
  ...
  function paint() {
    ...
    scrims = scrims || PlanScene.scrimBitmaps();
    ...
    scene = PlanScene.build(S.project, floor, S.library, theme, {
      ...
      flooring: S.flooring,
      when: S.when,
      /* Flat, like the dashboard card: on a desktop Edge over a D3D11 GPU the
       * plan's blend modes made the canvas tear and blank while it scrolled
       * and zoomed, and its masks made it crawl. The export builds its own
       * scene and keeps full compositing. */
      compositing: 'flat',
    });
    const scrimUrl = scrims.get(scene, () => requestAnimationFrame(paint));
    ...
    for (const key of scene.order) {
      ...
      const nodes = key === 'scrim' ? PlanScene.scrimLayer(scene, scrimUrl) : (scene.layers[key] || []);
      for (const n of nodes) g.appendChild(nodeToEl(n));
      frag.appendChild(g);
    }
     ...
     * must not there. */
    /* Without dashboard tap areas: a solar marker's tap area is its whole
     * array, and here it hid the array — every click on it selected the small
     * marker, so turning "the array" only ever turned the marker's icon. */
    for (const t of PlanScene.hitTargets(floor, S.library, scene.projector, S.states, null, { tapAreas: false })) {
  ...
  /* Where the knob sits: along the facing, `radius` out — pulled back along the
   * same line when that would leave the drawing. The SVG clips at its own edge,
   * so an array against the top of the floor had its knob drawn where nobody
   * could reach it. The drag reads only the angle, so a shorter arm turns it
   * exactly the same. `minR` keeps it off the centre it turns about. */
  function knobAt(cx, cy, radius, deg, width, height, margin, minR) {
    const a = deg * Math.PI / 180, dx = Math.sin(a), dy = -Math.cos(a);
    const room = (c, d, size) => (d > 1e-9 ? (size - margin - c) / d : d < -1e-9 ? (margin - c) / d : Infinity);
    const r = Math.max(minR, Math.min(radius, room(cx, dx, width), room(cy, dy, height)));
  ...
  function drawRotateHandle(ov, item, type, P) {
    ...
    const knobR = KNOB_PX() / (S.view.zoom || 1);
    const [hx, hy] = knobAt(cx, cy, radius, deg,
      scene ? scene.width : Infinity, scene ? scene.height : Infinity, knobR + 4 / (S.view.zoom || 1), knobR * 3);
  ...
  function placeAperture(hit) {
    ...
    Store.mutate(() => {
      ...
      for (const k of ['h', 'sill', 'swing', 'hinge', 'leaves', 'leafRatio', 'slideTo', 'trackFt', 'depth', 'curtain']) {
  ...
  return {
    ...
    furnitureBox, resizedBox, frontAngle, knobAt,
  };
}());
