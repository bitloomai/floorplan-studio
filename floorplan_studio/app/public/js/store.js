...
window.Store = (function () {
  ...
  /* The copy this tab last had from the server — what a save names as its
   * base, and what a merge compares both sides against. Set on load, after
   * every save, and whenever a project arrives from elsewhere. */
  function setSynced(project) {
    S.synced = project ? clone(project) : null;
  }

  /* A merge of this tab's edits with a newer project from elsewhere. Not an
   * undo entry and not `replaceProject`: the result holds unsaved local edits,
   * so it stays dirty and the 'project' event saves it. */
  function adoptMerged(project) {
    preserveDeployment();
    S.project = project;
    preserveDeployment();
    if (!floor()) S.activeFloorId = (S.project.floors[0] || {}).id || null;
  ...
  return {
    ...
    mutate, undo, redo, replaceProject, setSynced, adoptMerged,
