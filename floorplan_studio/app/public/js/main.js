...
(function () {
  ...
  async function save() {
    ...
    setSaveState('saving');
    /* A snapshot, not the live object: an edit made while the request is in
     * flight must not be recorded as already on the server. */
    const sent = JSON.parse(JSON.stringify(S.project));
    try {
      const r = await API.saveProject(sent, S.synced && S.synced.savedAt);
      Store.setSynced(Object.assign(sent, { savedAt: r && r.savedAt }));
      ...
      /* Written elsewhere since this tab loaded it: merge this tab's edits onto
       * the newer project and save THAT (adoptMerged schedules it), rather than
       * overwriting what an assistant or another tab just did. */
      if (e.status === 409 && e.body && e.body.project) {
        const theirs = e.body.project;
        const { project, conflicts } = ProjectMerge.merge(S.synced, S.project, theirs);
        Store.setSynced(theirs);
        setSaveState('dirty');
        Store.adoptMerged(project);
        Panels.toast(conflicts.length
          ? `The plan changed elsewhere; merged, keeping your version where you both changed the same thing (${conflicts.length}).`
          : 'The plan changed elsewhere; merged your edits with it.', conflicts.length > 0);
        return;
      }
  ...
  function watchRemoteChanges() {
    ...
    /* Bring this tab level with the project on the server. Replaced outright
     * when nothing here is unsaved; MERGED when something is — this used to
     * only toast, and the next save then wrote the stale copy over the change
     * (an assistant's rotation of the viewing deck's array, three times). A
     * note being typed is anchored to what is on screen, so that one waits; the
     * next save's refusal merges it then. */
    const catchUp = () => {
      clearTimeout(fetchTimer);
      fetchTimer = setTimeout(async () => {
        try {
          const project = await API.project();
          if (S.synced && project.savedAt && project.savedAt === S.synced.savedAt) return;
          if (NotesUI.active()) {
            const now = Date.now();
            if (now - lastNudge > 5000) {
              lastNudge = now;
              Panels.toast('The plan changed elsewhere; it will be merged in when you finish this note.', true);
            }
            return;
          }
          if (!S.dirty) { Store.replaceProject(project); return; }
          const { project: merged, conflicts } = ProjectMerge.merge(S.synced, S.project, project);
          Store.setSynced(project);
          Store.adoptMerged(merged);
          Panels.toast(conflicts.length
            ? `The plan changed elsewhere; merged, keeping your version where you both changed the same thing (${conflicts.length}).`
            : 'The plan changed elsewhere; merged it with your edits.', conflicts.length > 0);
        } catch (e) { /* the next event, or the next save's refusal, tries again */ }
      }, 300);
    };
     ...
     * gone — the add-on restarting is the usual cause — so the first open after
     * a drop checks the project instead of waiting for a change that already
     * happened. */
    let dropped = false;
    es.addEventListener('error', () => { dropped = true; });
    es.addEventListener('open', () => { if (dropped) { dropped = false; catchUp(); } });
  }
