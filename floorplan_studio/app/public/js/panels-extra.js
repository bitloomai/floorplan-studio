...
window.PanelsExtra = (function () {
  ...
  function roomExtras(box, floor, room) {
    ...
    const matched = Controls.matchedDomains(scenesSec);
    const matchedNames = matched.map((d) => d.endsWith('s') ? d : d + 's');
    const matchedPhrase = matchedNames.length > 1
      ? matchedNames.slice(0, -1).join(', ') + ' and ' + matchedNames.at(-1)
      : (matchedNames[0] || 'entities');
    ...
    box.appendChild(settingHelp('Leave blank to match the room’s name and id.', 'When to add match keys',
      `Matching ${matchedPhrase} join this room’s controls automatically.`,
  ...
  function renderOpeningPanel(box, floor, op) {
    ...
    box.appendChild(field('Type', h('select', {
      onchange: (e) => Store.mutate(() => {
        ...
        for (const k of ['swing', 'hinge', 'leaves', 'leafRatio', 'slideTo', 'trackFt', 'depth']) delete op[k];
    ...
    if (drawsTravel) {
      ...
      if (/slide|pocket|telescopic|scissor|side_sectional/.test(style)) {
        ...
        /* A gate that runs along the house side of its wall and parks behind a
         * pier is not on the opening's line: its rail is set back into the room. */
        if (style === 'slide' || style === 'telescopic') {
          box.appendChild(field('Rail set back into this room (ft)', numInput(op.trackFt ?? props.trackFt ?? 0,
            (v) => Store.mutate(() => { if (!v) delete op.trackFt; else op.trackFt = Math.max(0, v); }, 'rail set back'), 0.25)));
          box.appendChild(h('p', { class: 'hint' }, '0 runs the leaf on the opening’s own line. A compound-wall gate that slides behind a pier runs just past the pier’s thickness.'));
        }
      }
  ...
  function sunDialog() {
    ...
    A.adv(body, () => {
      ...
      body.appendChild(numIn('Sky share an outdoor room takes', 'ambient', 'outdoor', 0.05,
        '1 is open sky. Lower it for a terrace that is open above but overshadowed. A roofed car port says 0 in its own room panel.'));
      body.appendChild(numIn('How deep daylight reaches into a roofed space (× opening height)', 'ambient', 'depth', 0.1,
        'Light from a window or an open side falls off with distance from it, so a deep car port is bright at its open side and dim against the house.'));
